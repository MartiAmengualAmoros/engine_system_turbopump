...
% SUB_TURBINE_LCH4  Multi-stage turbine — LCH4 shaft.
% Stage count auto-computed to satisfy Ns >= Ns_min AND D_mean <= D_mean_max_LCH4
% Efficiency mode controlled by inputs.eta_turbine_mode:
%   'fixed'  -> uses inputs.eta_turbine_LCH4 directly
...
if delta_h_is_total <= 0
    key_values = zero_kv_LCH4(inputs, T_in);
...
%% 3. AUTO STAGE COUNT
omega         = inputs.N_shaft_LCH4 * pi / 30;
C0_full       = sqrt(2 * delta_h_is_total);
Ns_1stage     = (inputs.N_shaft_LCH4/60) * sqrt(Q_vol) / delta_h_is_total^0.75;
D_mean_1stage = 2 * inputs.nu_target_LCH4 * C0_full / omega;

Ns_min  = 0.08;
n_Ns    = ceil((Ns_min / max(Ns_1stage, 1e-12))^(1/0.75));
n_D     = ceil((D_mean_1stage / inputs.D_mean_max_LCH4)^2);
n_stages = max([n_Ns, n_D, 1]);

%% 4. PER-STAGE GEOMETRY
delta_h_stage = delta_h_is_total / n_stages;
C0_stage = sqrt(2 * delta_h_stage);
u1       = inputs.nu_target_LCH4 * C0_stage;
D_rotor  = 2 * u1 / omega;
r1       = D_rotor / 2;
...
if HTR_sq <= 0 || HTR_sq >= 1
    HTR_ex = 0.40;
    warning('sub_Turbine_LCH4: HTR out of range (HTR^2=%.3f), using fallback 0.40', HTR_sq);
else
...
%% 6. Ns PER STAGE
Ns_stage   = (inputs.N_shaft_LCH4/60) * sqrt(Q_vol) / delta_h_stage^0.75;
...
if isfield(inputs,'eta_turbine_mode') && strcmp(inputs.eta_turbine_mode,'balje')
    ...
    fprintf('[T-LCH4] Balje mode: Ns=%.4f  eta_Ns_max=%.3f  eta_used=%.3f\n', ...
        Ns_stage, eta_Ns_max, eta_turbine);
else
    eta_turbine = inputs.eta_turbine_LCH4;
    if eta_turbine > eta_Ns_max + 0.03
        warning('sub_Turbine_LCH4: eta_fixed=%.3f may be optimistic for Ns=%.4f (Balje max~%.3f).', ...
...
if strcmp(turbine_type,'Radial inflow turbine (RIT)') || ...
   strcmp(turbine_type,'Radial inflow turbine (RIT) - partial admission')
    if nu_actual < 0.50 || nu_actual > 0.75
        warning('sub_Turbine_LCH4: nu=%.3f outside optimal RIT range [0.50-0.75].', nu_actual);
    end
elseif contains(turbine_type,'Axial')
    if nu_actual < 0.35 || nu_actual > 0.60
        warning('sub_Turbine_LCH4: nu=%.3f outside optimal axial range [0.35-0.60].', nu_actual);
    end
end
...
surplus = W_available - inputs.P_turbine_LCH4_needed;
...
function kv = zero_kv_LCH4(inputs, T_in)
...
kv.surplus      = -inputs.P_turbine_LCH4_needed;
kv.T_out        = T_in;
