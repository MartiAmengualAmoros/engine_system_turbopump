...
%% 6.1 Slip Factor & Ángulo Físico de la Pala 

% 1. Selección del número de álabes (Gülich recomienda 5 a 7 para etapas radiales)
N_B_R = 6; 

% 2. Ratio del diámetro medio
delta_M = (delta_t + delta_h) / 2;

% 3. Bucle iterativo para el Slip Factor
beta2B_guess = beta2; % Valor semilla (usamos el ángulo cinemático en radianes)
error_beta = 1;
iter = 0;
...
while error_beta > 1e-5 && iter < 100
    ...
    % Fórmula clásica de Wiesner (usando el ángulo en radianes)
    SF_wiesner = 1 - (sqrt(cos(beta2B_guess)) / (N_B_R^0.7));
    
    % Corrección de Gülich para límites de diámetro
    ...
    % Cálculo de la velocidad tangencial ideal (con infinitos álabes)
    c2u_inf = c2u / SF;
    
    % Recálculo del ángulo geométrico de la pala beta2B
    % tan(beta2B) = (u2 - c2u_inf) / c2m
    beta2B_new = atan((u2 - c2u_inf) / c2m);
    
    % Actualización de error y variable
...
% Guardar resultados en grados para exportación al CAD
